...
//Забележка: 
//1. Под триене на указател (при оператора делете) имам предвид зануляването му до 00008123(това е настройка на visual studio, 
//просто при оператора delete не занулява указателя до nullptr (00000000), а до този адрес 00008123  )
//2. Под триене на масив (при delete), имам предвид освобождаване на паметта, към която указателят сочи.
